...
if __name__ == '__main__':
    loadpath = 'D:/PythonProjects_Data/Experiment/SimpleCNN/'

    MAEList, RMSEList, CCCList = [], [], []
    for filename in os.listdir(loadpath):
        ...
        CCCList.append(CCC_Calculation(label=data[:, 0], predict=data[:, 1]))
    print('MAE = %.2f\tRMSE = %.2f\tCCC = %.2f' % (min(MAEList), min(RMSEList), max(CCCList)))
